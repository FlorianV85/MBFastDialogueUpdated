...
namespace MBFastDialogue
{
    internal static class EncounterMenuEvaluator
    {
        public static string? GetEncounterMenu(PartyBase encountered)
        {
            ...
            if (encountered.IsSettlement || encountered.MapEvent != null)
                return null;

            var partyId = encountered.Id;
            var mobile = encountered.MobileParty;

            if (partyId.Contains(PartyIds.LocateAndRescueTravellerQuestRaiderParty))
                return null;

            if (
                partyId.Contains(PartyIds.QuestPartyTemplate)
                && mobile is { IsCurrentlyUsedByAQuest: true }
            )
                return null;

            if (
                partyId.Contains(PartyIds.NavalCorsair)
                && mobile is { IsCurrentlyUsedByAQuest: true }
            )
                return null;

            if (!instance.IsPatternWhitelisted(partyId))
                return null;
            ...
            if (
                encounteredFaction == mainFaction
            ...
            if (mobileParty?.IsCurrentlyUsedByAQuest == true && partyId.Contains(PartyIds.Villager))
                return null;

            if (!encountered.IsMobile)
                return ModuleConstants.FastEncounterMenu;

            if (mobileParty != null)
            {
                var mainMobile = MobileParty.MainParty;
                var isGarrisonWithSiege =
                    mobileParty.IsGarrison && mainMobile.BesiegedSettlement != null;
                var isOwnBesiegedSettlement =
                    mainMobile.CurrentSettlement != null
